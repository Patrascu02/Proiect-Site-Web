DROP TYPE IF EXISTS categ_prajitura;/*resetare de tabel*/
...
CREATE TYPE tipuri_produse AS ENUM('cofetarie', 'patiserie', 'gelaterie');/*pt fiecare specificator enum asoc un intreg*/


CREATE TABLE IF NOT EXISTS prajituri (--daca nu exista tabelul il creeam 
   ...
   gramaj INT NOT NULL CHECK (gramaj>=0),--sa fim siguri ca prajitura nu are gramaj negativ
   tip_produs tipuri_produse DEFAULT 'cofetarie',--default e daca nu punem nimic el pune autom cofetarie
   calorii INT NOT NULL CHECK (calorii>=0),
   categorie categ_prajitura DEFAULT 'comuna',
   ingrediente VARCHAR [], --pot sa nu fie specificate deci nu punem NOT NULL
