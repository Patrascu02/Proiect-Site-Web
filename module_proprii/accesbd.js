...
class AccesBD{
    ...
    constructor() {//pt conexiune unica
        if(AccesBD.#instanta){
    ...
    select({tabel="",campuri=[],conditiiAnd=[]} = {}, callback, parametriQuery=[]){
        let conditieWhere="";
        if(conditiiAnd.length>0)//daca am cond
            conditieWhere=`where ${conditiiAnd.join(" and ")}`; //vector de conditii 
        let comanda=`select ${campuri.join(",")} from ${tabel} ${conditieWhere}`;//selectez campurile din tabel cu conditiile de where
...
module.exports=AccesBD;//ce anume din acest fisier vr sa export