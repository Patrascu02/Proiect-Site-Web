...
class Utilizator{
    ...
    static parolaCriptare="475RADU1";
    static emailServer="sneakers4you444@gmail.com";
    ...
    constructor({id, username, nume, prenume, email, parola, rol, culoare_chat="black", poza}={}) {
        ...
        for(let prop in arguments[0]){//arg vectorul de argumente din constructor
            this[prop]=arguments[0][prop]//pt fiecare prop din arg setez val din argument
        }
        if(this.rol)
    ...
    */
    ...
    salvareUtilizator(){
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).insert({tabel:Utilizator.tabel,
            ...
            `<h1>Salut!</h1>
            <p style='color: blue;'>Username-ul tău este ${utiliz.username} pe site-ul ${Utilizator.numeDomeniu}.</p> 
...
//xjxwhotvuuturmqm
    ...
    static async getUtilizDupaUsernameAsync(username){
        if (!username) return null;//daca n am trnamsis niciun username
        try{
    ...
    static getUtilizDupaUsername (username,obparam, proceseazaUtiliz){
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).select({tabel:"utilizatori",campuri:['*'],conditiiAnd:[`username='${username}'`]}, function (err, rezSelect){
            ...
            else if(rezSelect.rowCount==0){//daca nu am utiliz cu username acela deci putem sa l inregistram
                eroare=-1;
            }
            //constructor({id, username, nume, prenume, email, rol, culoare_chat="black", poza}={})
            let u= new Utilizator(rezSelect.rows[0])//utiliz nou cu rez selectului 
